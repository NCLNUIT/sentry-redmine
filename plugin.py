...
#        ]
        ...
        #Request Headers
        output.append('\nh2. Headers\n')
        for k, v in event.data['sentry.interfaces.Http']['headers'].items():
            output.append('* *' + str(k) + ':* @' + str(v) + '@')

        #Request ENV (From Server)
        output.append('\nh2. Environment\n')
        for k, v in event.data['sentry.interfaces.Http']['env'].items():
            output.append('* *' + str(k) + ':* @' + str(v) + '@')

        #Request Cookies (Client)
        output.append('\nh2. Cookies\n')
        for k, v in event.data['sentry.interfaces.Http']['cookies'].items():
            output.append('* *' + str(k) + ':* @' + str(v) + '@')
